...
// Detectar si el directorio es escribible (falla en Render)
function resolveDbPath() {
  const localPath = path.join(__dirname, 'sportdata.json');
  try {
    fs.accessSync(__dirname, fs.constants.W_OK);
    return localPath;  // local: carpeta escribible
  } catch {
    return '/tmp/sportdata.json';  // Render u otros: usar /tmp
  }
}
