...
/* ── Config ─────────────────────────────── */
// En Render usar /tmp para escritura; local usar database/
const CACHE_FILE = process.env.RENDER
  ? '/tmp/products_cache.json'
  : path.join(__dirname, '..', 'database', 'products_cache.json');

// Caché inicial incluida en el repo (siempre disponible, read-only)
const SEED_CACHE_FILE = path.join(__dirname, '..', 'database', 'products_cache.json');
